...
import { validateAmount, validateTimestamp, findTimeLockPDA } from "./types";
/**
...
export class TimeLockBuilders {
     ...
     */
    async buildSolInitialize(params) {
        validateTimestamp(params.unlockTimestamp);
        const [timeLockAccount] = await findTimeLockPDA(params.owner, params.unlockTimestamp, this.program.programId);
     ...
     */
    async buildTokenInitialize(params) {
        validateTimestamp(params.unlockTimestamp);
        const [timeLockAccount] = await findTimeLockPDA(params.owner, params.unlockTimestamp, this.program.programId);
        return await this.program.methods
     ...
     */
    async buildTokenDeposit(params) {
        validateAmount(params.amount);
        const signerKey = params.depositor || this.program.provider.wallet?.publicKey;
        if (!signerKey) {
            throw new Error('No wallet or depositor provided');
     ...
     */
    async buildTokenWithdraw(params) {
        return await this.program.methods
            .withdrawToken()
            .accounts({
            timeLockAccount: params.timeLockAccount,
     ...
     */
    async buildSolCreateAndDeposit(params) {
     ...
     */
    async buildTokenCreateAndDeposit(params, depositParams) {
